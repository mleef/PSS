    ...
    init: function(callback){
        var ready = function(){
            try {
                ...
                // if there is a donation field and it contains a calculation widget reference
                if ($$('input[id*="_donation"]').length > 0 && $$('input[id*="_donation"]')[0].getAttribute('data-custom-amount-field') > 0) {
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    checkMultipleUploads: function(){
        if($$('.form-upload-multiple').length > 0){
            ...
            script.src = "https://static-interlogyllc.netdna-ssl.com/file-uploader/fileuploader.js?v5";
...
{       // Event simulator
        ...
        $H(JotForm.fieldConditions).each(function(pair){
            ...
            $A(conds).each(function(cond){
                $A(cond.action).each(function(action) {
                    if(action.field === field && action.visibility.toLowerCase() === visibility && action.hasOwnProperty('currentlyTrue') && action.currentlyTrue) {
    ...
    */
    ...
    changePaymentStrings: function (text) {
        if ($('coupon-header')) {
            $('coupon-header').innerHTML = this.paymentTexts.couponEnter;
        }
        if ($('shipping-text')) {
            $('shipping-text').innerHTML = this.paymentTexts.shippingShipping + ':';
        }
        if ($('tax-text')) {
            $('tax-text').innerHTML = this.paymentTexts.taxTax + ':';
        }
        if ($('subtotal-text')) {
            $('subtotal-text').innerHTML = this.paymentTexts.totalSubtotal + ':';
        }
        if ($('total-text')) {
            $('total-text').innerHTML = this.paymentTexts.totalTotal + ':';
        }
    },
    /*
     * Handles custom amount payment with values from calculation widget
     */        
    ...
    applyCoupon: function(discount) {
        ...
        if (!discount.paymentType || (discount.paymentType && discount.paymentType === "product")) {
            ...
            $A(discount.products).each(function(pid){ 
                ...
                if (discount.apply === "all") {
                    JotForm.discounts[pid] += "-all";   // all  payments for the current subscription item
                } else {
                    JotForm.discounts[pid] += "-first"; // first payment only for the current subscription item
                }
